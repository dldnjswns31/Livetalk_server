...
export const signupValidator = (signupForm: {
  email: string;
  nickname: string;
  password: string;
}) => {
  ...
  if (!validator.isLength(signupForm.nickname, { max: 8 })) {
    return {
      isValid: false,
      message: USER_VALIDATION_ERRORS.INVALID_NICKNAME,
    };
  }
  if (!validator.isLength(signupForm.password, { min: 8 })) {
    return {
      isValid: false,
      message: USER_VALIDATION_ERRORS.INVALID_PASSWORD,
    };
